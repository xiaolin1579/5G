...
const char * const BITCOIN_CONF_FILENAME = "5g.conf";
const char * const BITCOIN_PID_FILENAME = "5gd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "5gd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\5G
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\5G
    // Mac: ~/Library/Application Support/5G
    // Unix: ~/.5g
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "5G";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/5G";
#else
    // Unix
    return pathRet / ".5G";
#endif
