...
int GetMinProto()
{
    if(chainActive.Height() >= Params().GetConsensus().nMnCheckChangeHeight && sporkManager.IsSporkActive(Spork::SPORK_10_MASTERNODE_PAY_UPDATED_NODES))
...
CAmount GetBlockSubsidy(int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    // Subsidy is cut in half every 260,000 blocks which will occur approximately every 180 days.
    nSubsidy >>= halvings;
    return nSubsidy;
}
